...
//	while(I2S_AD16_GET_WP() != 0);		   /* FIXME: why ? */
...

	{
		...
		if(cRP || cWP)
		{
			bufSize = (cWP + 32 )&(~0x1F); /*FixMe: Do 970 use 32 and 0x1F?? */
	
			ori_size = ithReadRegA(I2S_REG_OUT_LEN);
			
			ithWriteRegA(I2S_REG_OUT_LEN, bufSize-1);			
			
			//printf("	getObufLen1:[%04x,%04x]cWP=%x,bufsz=%x\n",reg_read16(I2S_REG_TX_BUF_SIZE_H),reg_read16(I2S_REG_TX_BUF_SIZE_L),cWP,bufSize); 			
	
			bufBase = ithReadRegA(I2S_REG_OUT_BASE1);
			oBuf = (u8*)bufBase;
#if 0 //FIXME: reset DAC memory, 970 maybe open			
			...
			//printf("	getObufLen2:[%04x,%04x]\n",reg_read16(I2S_REG_TX_BUF_SIZE_H),reg_read16(I2S_REG_TX_BUF_SIZE_L),cWP,ori_size);	
		}
	}
...
////970 tempoarily use
/* When sample rate=48K, 9850 and 970 have different behavior */
/* 9850: ZCLK ratio=0x11, AMCLK ratio=0x0, use PLL2 */
/* 970: ZCLK ratio=0x111, AMCLK ratio=0x2 use PLL3 */

	//only for WS=48k 

	ithWriteRegA(MMP_AUDIO_CLOCK_REG_3C, 0xf2aac804); //reset
//	ithWriteRegA(0xD800003c, 0xf2aac803); //why not ZCLK ratio = 0x11, 9850 is 0x11 for 48K
	usleep(10);
	ithWriteRegA(MMP_AUDIO_CLOCK_REG_3C, 0x02aac804); //reset	//ZCLK	PLL3_OUT1 / d (i2s	 3.072MHz,WS=48K)
	usleep(1);

	ithWriteRegA(MMP_AUDIO_CLOCK_REG_40, 0x002c801);		//AMCLK PLL3_OUT1 / 3 (AMCLK 12.288MHz)
