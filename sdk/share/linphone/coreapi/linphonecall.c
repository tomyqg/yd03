...
{    
    printf("[DAC]   play_lev    = %d\n",lp_config_get_int(lc->config,SOUND,"play_lev",50));
    printf("[DAC]   rec_lev     = %d\n",lp_config_get_int(lc->config,SOUND,"rec_lev",50));
 ...
 */    
    {
        ...
        lp_config_read_file(lc->config,"E:/linphonerc");
    #else
        lp_config_read_file(lc->config,"D:/linphonerc");
    #endif      
        engeneer_audio_set(audiostream,lc);        
#endif         
        ...
        #else
        audiostream->use_ec=lp_config_get_int(lc->config,SOUND,"echocancellation",2);
        #endif
#endif
...
static void linphone_call_start_audio_stream(LinphoneCall *call, const char *cname, bool_t muted, bool_t send_ringbacktone, bool_t use_arc){
    ...
    if (stream && stream->dir!=SalStreamInactive && stream->port!=0){
        ...
        if (used_pt!=-1){
            ...
            if (captcard) ms_snd_card_set_level(playcard,MS_SND_CARD_CAPTURE,lc->sound_conf.rec_lev);            
            ...
            if(lc->presence_mode == LinphoneStatusDoNotDisturb)
            {
                ...
                //char *path = "E:/test.wav";
                //linphone_call_set_audio_stream_graph(astream,AudioFromSoundRead,path);
