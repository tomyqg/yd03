...
#include "ite970/D_table.inc"
#include "ite970/A_table.inc"
...
static int _ite970_micin  = 0;
static int _ite970_linein = 0;
static int _ite970_spkout = 0;
static int _ite970_hpout  = 0;
static int _ite970_DA_running = 0;
static int _ite970_AD_running = 0;

static void _internal_DAC_970_reset(void);
static void _internal_DAC_970_power_on(void);
static void _internal_DAC_970_power_off(void);
static void _internal_DAC_970_unmute(void);
static void _internal_DAC_970_mute(void);
...
void itp_codec_playback_init(unsigned output)
{
    ...
    _internal_DAC_970_reset();
	_ite970_DA_running = 1;
...
{   // precent to step :ex  82% -> 0x90 
	...
	if(it970DA_perc_to_reg_tableSize != 100) {
		printf("ERROR# invalid it970DA_perc_to_reg_tableSize !\n");
		return;
	}
...
void itp_codec_playback_get_currvol(unsigned *currvolperc)
{
	...
	for(i=0; i<99; i++)
	{
		if((it970DA_perc_to_reg_table[i] >= current_volstep)
		&& (current_volstep > it970DA_perc_to_reg_table[i+1])) {
			*currvolperc = i;
			return;
		}
	}
...
{   // precent to step :ex  82% -> 0x90 
	...
	if(it970AD_perc_to_reg_tableSize != 100) {
		printf("ERROR# invalid it970AD_perc_to_reg_tableSize !\n");
		return;
	}
	if(target_micperc >= 100) { target_micperc = 99; }

	micstep = it970DA_perc_to_reg_table[target_micperc];
