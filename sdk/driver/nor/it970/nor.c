    ...
    NOR_ID      *id)
{
    ...
    {
        command[0] = JEDEC_READ_ID;
        if (mmpAxiSpiPioRead(norObject->port, iomode, command, 1, 0, 0, &id2, 3) == false)
        {
            result = false;
            break;
        }
        ...
        command[0] = READ_ID_CMD;
        if (mmpAxiSpiPioRead(norObject->port, iomode, command, 1, 0x0, 3, &id1, 2) == false)
        {
            result = false;
            break;
        }
    ...
    uint32_t        outDataBufferSize)
{
        ...
        if (norObject->context.use4BytesAddress == true)
        {
            ...
            while (remainsize)
            {
                ...
                if (cnt) //unalign address
                {
                    ...
                    result              = mmpAxiSpiPioWrite(norObject->port, iomode, commands, 1, &waddr, 4, wptr, 4 - cnt);
                    wptr                += (4 - cnt);
                    remainsize          -= (4 - cnt);
                ...
                else //align address
                {
                    ...
                    else
                    {
                        ...
                        result      = mmpAxiSpiPioWrite(norObject->port, iomode, commands, 1, &waddr, 4, wptr, remainsize);
                        waddr       += remainsize;
                        wptr        += remainsize;
                        remainsize  = 0;
                    }
        ...
        else
        {
            ...
            while (remainsize)
            {
                ...
                if (cnt) //unalign address
                {
                    ...
                    result              = mmpAxiSpiPioWrite(norObject->port, iomode, commands, 1, &waddr, 3, wptr, 4 - cnt);
                ...
                else //align address
                {
                    ...
                    else
                    {
                        ...
                        result      = mmpAxiSpiPioWrite(norObject->port, iomode, commands, 1, &waddr, 3, wptr, remainsize);
                        waddr       += remainsize;
                        wptr        += remainsize;
                        remainsize  = 0;
                    }
    ...
    case READ:
        ...
        if (norObject->context.use4BytesAddress == true)
        {
            ...
            {
                result = mmpAxiSpiDmaRead(norObject->port, iomode, commands, 1, &arg, 4, (uint8_t *)outDataBuffer, outDataBufferSize);
            }
            else
            {
                result = mmpAxiSpiPioRead(norObject->port, iomode, commands, 1, &arg, 4, (uint8_t *)outDataBuffer, outDataBufferSize);
            }
        }
        else
        {
            if (outDataBufferSize >= 16)
            {
                result = mmpAxiSpiDmaRead(norObject->port, iomode, commands, 1, &arg, 3, (uint8_t *)outDataBuffer, outDataBufferSize);
            }
            else
            {
                result = mmpAxiSpiPioRead(norObject->port, iomode, commands, 1, &arg, 3, (uint8_t *)outDataBuffer, outDataBufferSize);
            }
        }
    #else
        if (norObject->context.use4BytesAddress == true)
        {
            result = mmpAxiSpiPioRead(norObject->port, iomode, commands, commandDataSize, (uint8_t *)outDataBuffer, outDataBufferSize, 32);
        }
        else
        {
            result = mmpAxiSpiPioRead(norObject->port, iomode, commands, 4, (uint8_t *)outDataBuffer, outDataBufferSize, 32);
        }
    #endif
        break;
    ...
    case FAST_READ:
        ...
        if (norObject->context.use4BytesAddress == true)
        {
            result = mmpAxiSpiPioRead(norObject->port, iomode, commands, commandDataSize, (uint8_t *)outDataBuffer, outDataBufferSize, 8);
        }
        else
        {
            result = mmpAxiSpiPioRead(norObject->port, iomode, commands, 4, (uint8_t *)outDataBuffer, outDataBufferSize, 32);
    ...
    case READ_STATUS:
        ...
        mmpAxiSpiPioRead(norObject->port, iomode, commands, 1, 0, 0, (uint8_t *)outDataBuffer, outDataBufferSize);
        break;

    case READ_ID:
        //9Fh (M7-M0) (ID15-ID8) (ID7-ID0)
        commands[0] = JEDEC_READ_ID;
        result      = mmpAxiSpiPioRead(norObject->port, iomode, commands, 1, 0, 0, (uint8_t *)outDataBuffer, 5);
        break;
    ...
    case READ_DEVICE:
        ...
        commands[3] = arg >> 8;
        commands[4] = arg;
        result      = mmpAxiSpiPioRead(norObject->port, iomode, commands, 1, &arg, 3, (uint8_t *)outDataBuffer, 2);
        break;
    case EN4B_MODE:
        //B7h
        cmd         = EN4B_MODE;
    ...
    NOR_OBJECT *norObject)
{
    ...
    do
    {
        ...
        if (norSendCommand(norObject, READ_STATUS, 0, (uint32_t)&readStatus, 4) == false)
        {
...
#if defined (CFG_NOR_USE_AXISPI)
        result  = mmpAxiSpiInitialize(NOR_BUS, SPI_OP_MASTR, CPO_0_CPH_0, SPI_CLK_20M);
#else
...
#if 1         /* Enable Nor Quad Mode */
        norSendCommand( norObject,  WRITE_EN,       0,  0,              0);
        norSendCommand( norObject,  WRITE_STATUS,   0,  (0x40 << 24),   1);
        mmpAxiSpiSetDummyByte(NOR_BUS, 6);// Set Nor Quad mode dummy byte
        if (norWaitReady(norObject, 100))
        {
