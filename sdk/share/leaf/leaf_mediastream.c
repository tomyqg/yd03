...
void leaf_set_broadcast_audiostream(LeafCall *call,int rate,LinphoneAudioStreamFlow select_flow,char *filepath){
    ...
    if(select_flow == AudioBroadcastRecv) audiostream->send_graph = FALSE;
    if(select_flow == AudioBroadcastSend) audiostream->receive_graph = FALSE;
    audiostream->audiorate = rate;
    audiostream->use_codec = FALSE;
    audiostream->use_volsend=FALSE;
    audiostream->use_volrecv=FALSE;
    audiostream->use_mix=FALSE;
    audiostream->eq_MIC=FALSE;
    audiostream->eq_SPK=FALSE;
    audiostream->use_ec=FALSE;
    audiostream->filename = filepath;
    //AudioBroadcastRecv: [udprecv]-->[castor3sndWrite]-->speaker
    //AudioBroadcastSend: mic-->[castor3sndRead]-->[udpsend]
}
