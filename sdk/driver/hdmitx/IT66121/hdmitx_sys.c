...
// EDID
////////////////////////////////////////////////////////////////////////////////
#if 1//ndef PARSE_EDID		//2015.4.29, my.wei add for test
...
void Config_GeneralPurpose_Infoframe_66121(BYTE *p3DInfoFrame);
...
void InitHDMITX_Instance_66121(TXInput_Device inputDevice)
{
    switch (inputDevice)
    {
        case TXIN_HDMIRX:			
			...
			//InstanceData.bInputVideoSignalType |= T_MODE_SYNCEMB;//2015.3.19, my.wei add
#endif
    ...
    HDMITX_InitInstance_66121(&InstanceData);
    InitHDMITX_66121();
...
void
HDMITX_SetOutput_66121(unsigned int AudioSampleRate, unsigned int AudioChannelNum, unsigned int inputDevice)
{
    MMP_UINT32 id, i;
    ...
    {
        //SetOutputColorDepthPhase_66121(bOutputColorDepth,0);
    }
    else
    {
    ...
    SetupVideoInputSignal_66121(InstanceData.bInputVideoSignalType);
    ...
    ProgramDEGenModeByID_66121(id, InstanceData.bInputVideoSignalType | T_MODE_DEGEN, inputDevice);
	#endif
    #ifdef SUPPORT_SYNCEMBEDDED
    if(InstanceData.bInputVideoSignalType & T_MODE_SYNCEMB)
    {
        if( gtVIC != 0  )
        {
            ProgramSyncEmbeddedByVIC_66121(gtVIC,InstanceData.bInputVideoSignalType);
        }
        else
        {
            ProgramSyncEmbeddedByTiming_66121(&VTiming,InstanceData.bInputVideoSignalType);
        }

    }
    #endif

    EnableVideoOutput_66121(level,bInputColorMode,bOutputColorMode ,bHDMIMode);
    ...
    if( bHDMIMode )
    {
        ...
        //EnableHDCP_66121(TRUE);
        if( bAudioEnable )
        {
            //EnableAudioOutput(TMDSClock, bAudioSampleFreq, OUTPUT_CHANNEL,24, FALSE);
#ifdef SUPPORT_HBR_AUDIO
            EnableHDMIAudio_66121(T_AUDIO_HBR, FALSE, 768000L,OUTPUT_CHANNEL,NULL,TMDSClock);
#else
            EnableHDMIAudio_66121(T_AUDIO_LPCM, EnSPDIFIn, (ULONG)AudioSampleRate, (BYTE)AudioChannelNum, NULL, TMDSClock);
#endif
            ConfigAudioInfoFrm();
        }
    }
    ...
    {
        EnableAVIInfoFrame_66121(FALSE ,NULL);
    }
    ...
    for (i = 0; i < 3; i++)
    {
        hdcp_done = EnableHDCP_66121(bEnableHDCP);
        if (hdcp_done)
        {
...
void
HDMITX_DevLoopProc_66121(unsigned char bHDMIRxModeChange, unsigned int AudioSampleRate, unsigned int AudioChannelNum,
                    TXInput_Device inputDevice)
{
    unsigned long TMDSClock = VideoPixelClock*(pixelrep+1);

    CheckHDMITX_66121(&HPDStatus,&HPDChangeStatus);
	...
	if( HPDChangeStatus )
    {
        ...
        {        	
            ...
            #ifdef SUPPORT_HDCP
                EnableHDCP_66121(FALSE);
            #endif
			}
    		HDMITX_DEBUG_PRINTF(("HPD change HDMITX_SetOutput_66121();\n"));
            //HDMITX_SetOutput_66121(AudioSampleRate, AudioChannelNum, inputDevice);
            bChangeMode=TRUE;
        ...
        {
            // unplug mode, ...
    		HDMITX_DEBUG_PRINTF(("HPD OFF HDMITX_DisableVideoOutput_66121()\n"));
            DisableVideoOutput_66121();
            HDMITX_PowerDown();
        }
    }
...
void
HDMITX_ChangeDisplayOption_66121(HDMI_Video_Type OutputVideoTiming, MODE_ID VesaTiming, unsigned int EnableHDCP, BOOL IsYUVInput)
{
...
//     AVI_InfoFrame AviInfo;
    ...
    EnableAVIInfoFrame_66121(TRUE, (unsigned char *)&AviInfo);
}
...
ConfigAudioInfoFrm()
{
    ...
    EnableAudioInfoFrame_66121(TRUE, (unsigned char *)&AudioInfo);
}
...
ParseEDID_66121()
{
...
#ifdef PARSE_EDID
#if 0	
    GetEDIDData_66121(0, EDID_Buf);
#else	//2015.4.29, my.wei add for test
    if(FALSE == GetEDIDData_66121(0, EDID_Buf))//;
    	{
			HDMITX_DEBUG_PRINTF(("GetEDIDData_66121 ERR, copy block0\r\n"));
    ...
    {
#ifdef PARSE_EDID
        err = GetEDIDData_66121(i, EDID_Buf);
#if 1	//2015.4.29, my.wei add for test
		if(err == FALSE)
		{
			HDMITX_DEBUG_PRINTF(("GetEDIDData_66121 ERR, copy block1\r\n"));
...
void Config_GeneralPurpose_Infoframe_66121(BYTE *p3DInfoFrame)
{
    ...
    EnableVendorSpecificInfoFrame_66121(TRUE, p3DInfoFrame) ;

}



void HDMITXDisable_66121 (void)
{
    //DisableAudioOutput_66121();
    DisableVideoOutput_66121();
}
...
BYTE HDMITX_ReadI2C_Byte_66121(BYTE RegAddr)
{
    ...
    //mmpIicReleaseModule();
    usleep(1000); //avoid AV no sync, don't remove
    return d;
}


SYS_STATUS HDMITX_WriteI2C_Byte_66121(BYTE RegAddr, BYTE d)
{
    BOOL flag;
...
SYS_STATUS HDMITX_SetI2C_Byte(BYTE Reg,BYTE Mask,BYTE Value)
{
    ...
    if( Mask != 0xFF )
    {
        Temp=HDMITX_ReadI2C_Byte_66121(Reg);
        Temp&=(~Mask);
        Temp|=Value&Mask;
    }
    else
    ...
    return HDMITX_WriteI2C_Byte_66121(Reg,Temp);
}
