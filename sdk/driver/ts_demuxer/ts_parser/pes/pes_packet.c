    ...
    void*               pCallbackData)
{
    ...
    // pRiscBaseAddr: (plug-in lib, RISC start memory address)
    ptQueue = pesQm_CreateQueue(channelId, pesIndex, ptPidInfo, pRiscBaseAddr);
    if (MMP_NULL == ptQueue)
        return MMP_NULL;
    ...
    if (ptDecoder)
    {
        ...
        ptDecoder->queueSampleSize  = ptQueue->pesOutSampleSize;
        ...
        ptDecoder->tBuildingPes.pesIndex = pesIndex;
    }
    return ptDecoder;
}
    ...
    PES_DECODER*    ptDecoder)
{
    ...
    pesQm_DestroyQueue(ptDecoder->ptQueue);
    ...
    uint8*          pPacketAddress)
{
    ...
    if (payload_unit_start_indicator)
    {
        ...
        if (MMP_SUCCESS != pesQm_GetFree(ptDecoder->ptQueue, (void**)&ptDecoder->ptBuildingPes->pData))
        {
            ptDecoder->ptBuildingPes = MMP_NULL;
            return;
        }
    ...
    PES_DECODER*    ptDecoder)
{
    ...
    while (availableByte > 0)
    {
        ...
        else // ptDecoder->priorHeaderReadByte == PES_PRIOR_HEADER_SIZE.
        {
            ...
            {
                // video
                // 將 _PES_CreateSegment() 改成 ring alloc
                if (!_PES_CollectSegment(ptDecoder, availableByte, pPayloadAddress))
                {
                    // 失敗時的error handling
                }
    ...
    PES_DECODER*    ptDecoder)
{
    ...
    pesQm_SetReady(ptDecoder->ptQueue, (void**)&ptDecoder->ptBuildingPes->pData, &ptDecoder->ptBuildingPes->gatherSize);
