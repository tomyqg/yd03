...
static uint8_t ADV7180_IICADDR = 0x40 >> 1; //hw select default 0x42
#else
...
static uint8_t _ADV7180_ReadI2c_Byte(uint8_t RegAddr)
{
    ...
    {
        ithPrintf("ADV7180 I2C Read error, reg = %02x\n", RegAddr);
...
static uint32_t _ADV7180_WriteI2c_Byte(uint8_t RegAddr, uint8_t data)
{
    uint32_t flag;
    ...
    {
        ithPrintf("ADV7180 I2c write error, reg = %02x val =%02x\n", RegAddr, data);
        mmpIicGenStop(IIC_PORT_0);
    }
    //mmpIicReleaseModule(IIC_PORT_0);
    return flag;
...
static uint32_t _ADV7180_WriteI2c_ByteMask(uint8_t RegAddr, uint8_t data, uint8_t mask)
{
    uint8_t    value;
    uint32_t   flag;
...
{
    ithPrintf("ADV7180Initialize here\n");
...
uint16_t ADV7180GetProperty(MODULE_GETPROPERTY property)
{
    ...
    {
       if ((ADV7180_InWidth == ADV7180_TABLE[i].HActive) && (ADV7180_InHeight == ADV7180_TABLE[i].VActive) && ADV7180_InFrameRate == ADV7180_TABLE[i].Rate)
        {
        	index = i;
			matchRes = MMP_TRUE;
       	}
	...
	switch(property)
	{
		 ...
		 case GetModuleIsInterlace:
		    ...
		    break;
		 case FrameRate:
		 	return ADV7180_TABLE[index].FrameRate;
		    break;
	     case matchResolution:
		 	return matchRes;
		    break;
		 case HPolarity:
		 	return ADV7180_TABLE[index].HPolarity;
		 	break;
		 case VPolarity:
		 	return ADV7180_TABLE[index].VPolarity;
		 	break;
		 case HStar:
		 	return ADV7180_TABLE[index].HStar;
		 	break;
		 case HEnd:
		 	return ADV7180_TABLE[index].HEnd;
		 	break;
		 case VStar1:
		 	return ADV7180_TABLE[index].VStar1;
		 	break;
		 case VEnd1:
		 	return ADV7180_TABLE[index].VEnd1;
		 	break;
		 case VStar2:
		 	return ADV7180_TABLE[index].VStar2;
		 	break;
		 case VEnd2:
		 	return ADV7180_TABLE[index].VEnd2;
		 	break;
		 default:
