...
void itp_codec_wake_up(void)
{
	...
	/* config GPIO for depop */
	{
		...
		/* set as output mode */
		ithGpioSetOut(CFG_DAC_EXT_DEPOP_GPIO_PIN);;

		/* set to 1 */
		ithGpioSet(CFG_DAC_EXT_DEPOP_GPIO_PIN);
	...
	/* debug: read back */
	{
		...
		do
		{
			data32 = ithGpioGet(CFG_DAC_EXT_DEPOP_GPIO_PIN);
			__asm__("");
		}
		while(!data32);
...
//		reg_write16(I2S_REG_AMP_VOL, data16);
	...
	/* set to 0 */
	{
		ithGpioClear(CFG_DAC_EXT_DEPOP_GPIO_PIN);
...
//	}
	...
	/* workaround: Digital -2 dB to avoid amp bug */
	{
		/* enable_fading, fast response */
		{
			...
			data16 = reg_read16(I2S_REG_FADE_IN_OUT_CTRL);
			data16 |= (1 << 12);
