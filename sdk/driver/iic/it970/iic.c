    ...
    uint32_t     delay)
{
    ...
    do
    {
        if (IicDevices[port].refCount == 0)
        {
            ...
            // Init HW
            if (IicIniDeviceHw(port, initClock) == false)
            {
                IIC_ERROR_MSG("IicIniDeviceHw() fail.\n");
                break;
            }
...
#ifdef IIC_USE_DMA
        result = IicFiFoDmaSendData(port, slaveAddr, pdbuf, pad_size);
#else
        result = IicFiFoSendData(port, slaveAddr, pdbuf, pad_size);
#endif
        if (result == 1)
        {
    ...
    uint32_t    outDataSize)
{
    ...
    {
#ifdef IIC_USE_DMA
        result = IicFiFoDmaSendData(port, slaveAddr, outData, outDataSize);
#else
        result = IicFiFoSendData(port, slaveAddr, outData, outDataSize);
#endif
        if (result == 1)
        {
            IIC_ERROR_MSG("Send data fail.\n");
    ...
    uint32_t    outputBufferLength)
{
    ...
    do
    {
        ...
        ithFlushDCacheRange(mappedSysRam, outputBufferLength * sizeof(uint16_t));
    ...
    uint32_t    clock)
{
    ...
    do
    {
        ...
        else if(IicDevices[port].port == IIC_PORT_1)
        {
        ...
        else if(IicDevices[port].port == IIC_PORT_2)
        {
            ithWriteRegA(ITH_GPIO_BASE + 0x238, (1 << 15) | (1 << 31) | (CFG_GPIO_IIC0_DATA << 16) | (CFG_GPIO_IIC0_CLK));
        }
#endif
#ifdef CFG_I2C1_ENABLE
        else if(IicDevices[port].port == IIC_PORT_3)
        {
            ithWriteRegA(ITH_GPIO_BASE + 0x23C, (1 << 15) | (1 << 31) | (CFG_GPIO_IIC1_DATA << 16) | (CFG_GPIO_IIC1_CLK));
        }
#endif
    ...
    if (IIC_REG_BIT(regData, 28) == 1)  /* REQUEST DOWN */
    {
        if ( IIC_REG_BIT(regData, 0) == 0 ) /* W */
        {
            printf("Slave in READ\n");
            {
                ...
                {
                    readData[i] = ithReadRegA(IicDevices[port].regAddrBase + REG_I2C_DATA);
                    readData[i] = readData[i] & 0xFF;
                    outputBufferWriteIndex++;
                }
                if ( IicDevices[port].recvCallback )
                {
                    IicDevices[port].recvCallback(readData, recvDataSize);
                }
                if (IIC_REG_BIT(regData, 29) == 1)
                    ithWriteRegA(IicDevices[port].regAddrBase + REG_I2C_STATUS,
                        REG_BIT_STATUS_DATA_END_CLEAR |
    ...
    uint32_t        size)
{
    ...
    do
    {
        ...
        ithFlushDCacheRange(mappedSysRam, (size + 1) * sizeof(uint16_t));
