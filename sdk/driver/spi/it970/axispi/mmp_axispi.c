    ...
    SPI_PORT port)
{
    ...
    static PORT_IO_MAPMING_ENTRY    tSpiWpMappingTable[] =
    {
        {SPI_0, 9, 1},
    };

    static PORT_IO_MAPMING_ENTRY    tSpiHoldMappingTable[] =
    {
        {SPI_0, 10, 1},
    };
    ...
    //WP
    entryCount                          = sizeof(tSpiWpMappingTable) / sizeof(SPI_IO_MAPPING_ENTRY);
    for (i = 0; i < entryCount; i++)
    {
        if (port == tSpiWpMappingTable[i].port
            && wpGpio == tSpiWpMappingTable[i].gpioPin)
        {
            break;
        }
    }
    ...
    ptMappingEntry->spiWp.gpioPin   = tSpiWpMappingTable[i].gpioPin;
    ptMappingEntry->spiWp.gpioMode  = tSpiWpMappingTable[i].gpioMode;

    //HOLD
    entryCount                      = sizeof(tSpiHoldMappingTable) / sizeof(SPI_IO_MAPPING_ENTRY);
    for (i = 0; i < entryCount; i++)
    {
        if (port == tSpiHoldMappingTable[i].port
            && holdGpio == tSpiHoldMappingTable[i].gpioPin)
        {
            break;
        }
    }
    ...
    ptMappingEntry->spiHold.gpioPin     = tSpiHoldMappingTable[i].gpioPin;
    ptMappingEntry->spiHold.gpioMode    = tSpiHoldMappingTable[i].gpioMode;
...
mmpAxiSpiDmaWrite(
    ...
    uint32_t        inDataSize)
{
    return mmpSpiPioWrite(port, iomode, inCommand, inCommandLen, slaveaddr, slaveaddrLen, inData, inDataSize);
}
#endif
    ...
    uint32_t        outDataSize)
{
    ...
    #if 1
    if ((iomode == SPI_IOMODE_4) || (iomode == SPI_IOMODE_2))
        AxiSpiWriteDummy(gDummyByte);

    AxiSpiWriteInstCode4bitRead((uint32_t)*inCommand, iomode);
    #else
...
mmpAxiSpiDmaRead(
    ...
    uint32_t        outDataSize)
{
    return mmpAxiSpiPioRead(port, mode, inCommand, inCommandLen, slaveaddr, slaveaddrLen, outData, outDataSize);
}
#endif
    ...
    uint32_t        outDataSize)
{
    ...
    if (((uint32_t)*inCommand == 0x05) || ((uint32_t)*inCommand == 0x0F))   // Read Nor/Nand Status cmd
    {
        if (outDataSize)
