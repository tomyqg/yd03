                ...
                if( pJFrmComp->decColorSpace != JPG_COLOR_SPACE_YUV411 )
                {
					...
					switch(pHJComm->pJDispInfo->outColorSpace)
					{
						...
						case JPG_COLOR_SPACE_ARGB4444:
							...
							pHJCodec->jAlphaPlane.bEnConstAlpha = false; //Note.bEnConstAlpha is true is ConstAlpha, bEnConstAlpha is false is per pixel Alpha.
						break;
						...
						pHJComm->pJDispInfo->outColorSpace == JPG_COLOR_SPACE_ARGB8888 )
					{
						...
						{
							pHJCodec->jAlphaPlane.ConstAlpha = 100; //just for test.
						}
						else
						{
							uint32_t   size = pJFrmComp->imgWidth * pJFrmComp->imgHeight;
							alpha_vram_addr = itpVmemAlignedAlloc(32, size); //32 bit aligen
							alpha_sys_addr = (uint8_t*) ithMapVram(alpha_vram_addr, size, ITH_VRAM_WRITE);
							if (alpha_vram_addr == NULL){printf("alpha_vram_addr open fail\n");}
							
							pHJCodec->jAlphaPlane.AlphaPlaneAddr  = alpha_sys_addr;
							pHJCodec->jAlphaPlane.AlphaPlanePitch = pJFrmComp->imgWidth;
							printf("pHJCodec->jAlphaPlane.AlphaPlaneAddr=0x%x\n",pHJCodec->jAlphaPlane.AlphaPlaneAddr);
							int i =0 , j =0;
							for(i=0; i<size; i++)
							{
								pHJCodec->jAlphaPlane.AlphaPlaneAddr[i] = j;
								j++;
								if(j==256) j=0;
							}
							ithUnmapVram(alpha_sys_addr ,size);
						}
					}
                }
                else
