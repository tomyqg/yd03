...
void HM1375Initialize(uint16_t Mode);
void HM1375Terminate(void);
void HM1375OutputPinTriState(unsigned char flag);
uint16_t HM1375GetProperty(MODULE_GETPROPERTY property);
bool HM1375GetStatus(MODULE_GETSTATUS Status);
void HM1375PowerDown(unsigned char enable);
void HM1375ForCaptureDriverSetting(CAP_CONTEXT *Capctxt );
unsigned char HM1375IsSignalStable(uint16_t Mode);
