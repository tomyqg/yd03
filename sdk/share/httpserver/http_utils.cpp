...
//#include <stdlib.h>
//#include <string.h>
#if defined(WIN32) || defined(__MINGW32__) || defined(__CYGWIN32__)
    //#define _WINDOWS
    #undef _WIN32_WINNT
...
#if HAVE_REGEX
    std::string            n_url;
    string_utilities::regex_replace(url, "(\\/)+", "/", n_url); // 將 "/a" or "////a" 取代成 "/a"
#else
    std::string            n_url        = url;
#endif
    std::string::size_type n_url_length = n_url.length();

    if (n_url_length > 1 && n_url[n_url_length - 1] == '/') // 去掉尾部的 "/"
    {
...
ip_representation::ip_representation(const struct sockaddr *ip)
{
    ...
    else
    {
        ...
        {
            pieces[i / 2] = ((u_char *)&(((struct sockaddr_in6 *)ip)->sin6_addr))[i] +
    ...
    if (ip.find(':') != std::string::npos) //IPV6
    {
        ...
        for (unsigned int i = 0; i < parts.size(); i++)
        {
            if (parts[i] != "*" && parts[i] != "")
            {
                ...
                if (parts[i].size() == 4)
                {
                    pieces[y]     = (unsigned short)strtol((parts[i].substr(0, 2)).c_str(), NULL, 16);
                    pieces[y + 1] = (unsigned short)strtol((parts[i].substr(2, 2)).c_str(), NULL, 16);
                    y += 2;
                }
                else
                {
                    ...
                    if (parts[i].find('.') != std::string::npos)
                    {
                        ...
                        if (subparts.size() == 4)
                        {
                            for (unsigned int ii = 0; ii < subparts.size(); ii++)
                            {
                                if (subparts[ii] != "*")
                                {
                                    pieces[y + ii] = (unsigned short)strtol(subparts[ii].c_str(), NULL, 10);
                                }
                                else
                                {
                                    CLEAR_BIT(mask, y + 11);
    ...
    else    //IPV4
    {
        ...
        if (parts.size() == 4)
        {
            for (unsigned int i = 0; i < parts.size(); i++)
            {
                if (parts[i] != "*")
                {
                    pieces[12 + i] = (unsigned short)strtol(parts[i].c_str(), NULL, 10);
                }
                else
                {
                    CLEAR_BIT(mask, 12 + i);
