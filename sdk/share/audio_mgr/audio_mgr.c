...
MMP_BOOL smtkSetQuickPlay(void)
{
    //HX : for quick play small wav file(64k)
    if (strcmp(gpNetwork.pFilename, audioMgr.ptNetwork.pFilename) == 0 && audioMgr.ptNetwork.nSpecialCase == 1)
    {
        audioMgr.bQuickPlay = MMP_TRUE;
        audioMgr.Nfilequeque++;
        return audioMgr.bQuickPlay;
    }
    else
    {
        audioMgr.bQuickPlay = MMP_FALSE;
...
static void *audio_read_thread_func(void *arg)
{
    ...
    for (;;)
    {
        ...
        if (audioMgr.Nfilequeque)   //init i2s need some time that may cause video lag,
        {
            ...
            if (audioMgr.Nfilequeque == 0 && !audioMgr.bQuickPlay)
            {
                pthread_mutex_lock(&audio_read_buffer_mutex);
                smtkAudioMgrQuickStop();
