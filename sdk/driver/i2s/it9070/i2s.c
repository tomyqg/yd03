...
extern void itp_codec_power_on(void);
extern void itp_codec_power_off(void);
...
uint16_t I2S_AD16_GET_RP(void) { return _I2S_GetRxBufRdPtr(); }
uint16_t I2S_AD16_GET_WP(void) { return _I2S_GetRxBufWrPtr(); }
void I2S_AD16_SET_RP(uint16_t data16) { _I2S_SetRxBufRdPtr(data16); }
    ...
    case 12000:
        {
            ...
            _REG_Write16(ITH_PLL2_SET1_REG, 0x8002);
            _REG_Write16(ITH_PLL2_SET2_REG, 0x3A3A); /* AMCLK select PLL2_OUT1 */
            _REG_Write16(ITH_PLL2_SET3_REG, 0x2000);
            _REG_Write16(ITH_PLL2_SET4_REG, 0x075A);
            _REG_Write16(ITH_PLL2_SET3_REG, 0x3000);
            ithDelay(100); /* unit: micro-sec (us) */ /* Experts Recommend Value: 100us */
            _REG_Write16(ITH_PLL2_SET3_REG, 0x2000);
    ...
    case 11025:
        {
            ...
            _REG_Write16(ITH_PLL2_SET3_REG, 0x2000);
            _REG_Write16(ITH_PLL2_SET4_REG, 0x2740);
            _REG_Write16(ITH_PLL2_SET3_REG, 0x3000);
            ithDelay(100); /* unit: micro-sec (us) */ /* Experts Recommend Value: 100us */
            _REG_Write16(ITH_PLL2_SET3_REG, 0x2000);
    ...
    case 8000:
        {
            ...
            _REG_Write16(ITH_PLL2_SET1_REG, 0xA003);
            _REG_Write16(ITH_PLL2_SET2_REG, 0x6565); /* AMCLK select PLL2_OUT1 */
            _REG_Write16(ITH_PLL2_SET3_REG, 0x2000);
            _REG_Write16(ITH_PLL2_SET4_REG, 0x3062);
            _REG_Write16(ITH_PLL2_SET3_REG, 0x3000);
            ithDelay(100); /* unit: micro-sec (us) */ /* Experts Recommend Value: 100us */
            _REG_Write16(ITH_PLL2_SET3_REG, 0x2000);
...
static void _i2s_use_GPIO_MODE2(void) /* GPIO settings for CFG2 */
{
    ...
    ithGpioSetDriving(7, ITH_GPIO_DRIVING_0);   /* 0:OK, 3:OK, but '0' is better */
}
...
//  printf("I2S# %s +\n", __func__);
    ...
    /* buffer base */
    {
        ...
        /* buf 1 (HDMI Data0) */
        _I2S_SetTxBufAddr(I2S_TX_BUF_1, (uint32_t)i2s_spec->base_hdmi[0]);
        /* buf 2 (HDMI Data1) */
        _I2S_SetTxBufAddr(I2S_TX_BUF_2, (uint32_t)i2s_spec->base_hdmi[1]);
        /* buf 3 (HDMI Data2) */
        _I2S_SetTxBufAddr(I2S_TX_BUF_3, (uint32_t)i2s_spec->base_hdmi[2]);
        /* buf 4 (HDMI Data3) */
        _I2S_SetTxBufAddr(I2S_TX_BUF_4, (uint32_t)i2s_spec->base_hdmi[3]);
        /* buf 5 (IIS / SPDIF Data) */
        _I2S_SetTxBufAddr(I2S_TX_BUF_5, (uint32_t)i2s_spec->base_i2s);
        /* buf 6 (SPDIF Data) */
        _I2S_SetTxBufAddr(I2S_TX_BUF_6, (uint32_t)i2s_spec->base_spdif);

        /* SPDIF data channel select */
        if(_g_UseSpdif)
        {
            if (i2s_spec->is_dac_spdif_same_buffer)
                _REG_ClrBit16(I2S_REG_SPDIF_VOL, I2S_POS_SEL_SPDIF_TX_BUF); /* 0:Output Buffer Base5 */
            else
                _REG_SetBit16(I2S_REG_SPDIF_VOL, I2S_POS_SEL_SPDIF_TX_BUF); /* 1:Output Buffer Base6 */
        }
    }
...
//  ithWriteRegA( 0xDE000000 | 0xD0, 0x0006); //set spdif clk from 1:pclk 0:amclk
    ithClearRegBitA(ITH_GPIO_BASE + ITH_GPIO_MISC_SET_REG, ITH_GPIO_SSP_CLK_SEL_POS);
