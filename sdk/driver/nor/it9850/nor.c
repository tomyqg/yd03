...
norSetByteMode(
	NOR_OBJECT*   norObject,
	NOR_BYTE_MODE byteMode)
{
	bool	result = true;

	do{
		if (byteMode == NOR_EN3B)
		{
			if (norSendCommand(norObject, EX4B_MODE, 0, 0, 0) == false)
			{
				NOR_ERROR_MSG("Send EX4B_MODE fail.\n");
				result = false;
				break;
			}
		}
		else if (byteMode == NOR_EN4B)
		{
			if (norSendCommand(norObject, EN4B_MODE, 0, 0, 0) == false)
			{
				NOR_ERROR_MSG("Send EN4B_MODE fail.\n");
				result = false;
				break;
			}
		}
	...
	uint32_t    eraseLength)
{
	...
	do{
		...
		if (norObject->context.use4BytesAddress)
		{
			if (norSetByteMode(norObject, NOR_EN4B) == false)
			{
				NOR_ERROR_MSG("norSetByteMode(%d) fail.\n", NOR_EN4B);
				result = false;
				break;
			}
		}
		...
		if (norObject->context.use4BytesAddress)
		{
			if (norSetByteMode(norObject, NOR_EN3B) == false)
			{
				NOR_ERROR_MSG("norSetByteMode(%d) fail.\n", NOR_EN3B);
				result = false;
				break;
