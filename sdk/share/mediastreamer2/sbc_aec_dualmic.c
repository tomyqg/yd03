...
typedef struct SbcAECState{
    ...
    MSBufferizer hw_echoaux;    
    MSBufferizer mic_stero_in;
    MSBufferizer echo_aux;
...
void AEC_IN_ARM_INIT(int *AECframesize,int *AECdelayms){
    ...
    *AECframesize = 256;
    *AECdelayms   = CFG_AEC_DELAY_MS - 15;
    #else
    *AECframesize = 128;
    *AECdelayms   = CFG_AEC_DELAY_MS - 15;//HX: there are too many Kconfig files to be modify
    #endif                                //(CFG_AEC_DELAY_MS - 15) will be fine delay
}
...
static void sbc_aec_init(MSFilter *f){
    ...
    ms_bufferizer_init(&s->hw_echoaux);
    ms_bufferizer_init(&s->mic_stero_in);
    ms_bufferizer_init(&s->echo_aux);
...
static void sbc_aec_process(MSFilter *f){
    ...
    if (s->bypass_mode) {
        ...
        if(s->SteroChannel){
            mblk_t *m;
            uint8_t buffer[2048];
            int size_of_pcm = 2*nbytes;
            while((m=ms_queue_get(f->inputs[1]))!=NULL){
                ms_bufferizer_put(&s->mic_stero_in,m);
            }

            while (ms_bufferizer_read(&s->mic_stero_in,buffer,size_of_pcm)==size_of_pcm){
                mblk_t *micL = allocb(nbytes,0);
                mblk_t *micR = allocb(nbytes,0);
                int i;

                for (i=0;i<size_of_pcm/2;i+=2){
                    *((int16_t*)(micL->b_wptr))=((int16_t*)buffer)[i];
                    *((int16_t*)(micR->b_wptr))=((int16_t*)buffer)[i+1];
                    micL->b_wptr+=2;
                    micR->b_wptr+=2;
                }
                ms_queue_put(f->outputs[1],micL);
                free(micR);
                //ms_bufferizer_put(&s->echo,micL);
                //ms_bufferizer_put(&s->echo_aux,micR);
            }        
        }else{        
    ...
    echoaux=(uint8_t*)alloca(nbytes);
    while ((ms_bufferizer_read(&s->echo,echo,nbytes)>=nbytes)&&
           (ms_bufferizer_read(&s->echo_aux,echoaux,nbytes)>=nbytes)){
        int avail;
        int avail_samples;
        mblk_t *clean;
        mblk_t *echo_to_ec;
        mblk_t *ref_to_ec;
        mblk_t *echoaux_to_ec;
        ...
        echoaux_to_ec = allocb(nbytes, 0);        
        memcpy(echoaux_to_ec->b_wptr, echoaux, nbytes);
        echoaux_to_ec->b_wptr += nbytes;
        ...
        ms_bufferizer_put(&s->hw_echoaux,echoaux_to_ec);//put to buffer          
#ifdef EC_DUMP_ITE
        putq(&s->echo_copy_q, dupmsg(echo_to_ec));
        putq(&s->echoaux_copy_q, dupmsg(echoaux_to_ec));
...
static void sbc_aec_postprocess(MSFilter *f){
    ...
    ms_bufferizer_flush (&s->mic_stero_in);
    ms_bufferizer_flush (&s->echo_aux);
    ms_bufferizer_flush (&s->hw_echoaux);
    ...
    static int index = 0;
    char *fname;
    fname=ms_strdup_printf("e:/echo%03d.raw", index);
    echofile=fopen(fname,"w");
    ms_free(fname);
    fname=ms_strdup_printf("e:/echoaux%03d.raw", index);
    echoauxfile=fopen(fname,"w");
    ms_free(fname);
    fname=ms_strdup_printf("e:/ref%03d.raw", index);
    reffile=fopen(fname,"w");
    ms_free(fname);
    fname=ms_strdup_printf("e:/clean%03d.raw", index);
    cleanfile=fopen(fname,"w");
    ms_free(fname);
    index++;
    while (1)
    {
        echo=ref=echoaux=clean=NULL;
        echo=getq(&s->echo_copy_q);
        echoaux=getq(&s->echoaux_copy_q);
        ref=getq(&s->ref_copy_q);
        clean=getq(&s->clean_copy_q);
        if (echo && ref && clean && echoaux)
        {
            fwrite(echo->b_rptr,nbytes,1,echofile);
            freemsg(echo);
            fwrite(echoaux->b_rptr,nbytes,1,echoauxfile);
            freemsg(echoaux);                   
